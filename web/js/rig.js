...
function projCell(n, h) {   // Projectors table: power dot · source · ⏻ · input menu · ⚙ — two-way over RS-232 / PJLink from that Pi
  ...
  const out = (h.out || '1/1').split('/'), omode = out[0], ndisp = +out[1] || 1;
  const outSel = `<select class="btn small" data-pjout="${esc(n)}" title="which micro-HDMI port(s) this Pi drives · ${ndisp} display${ndisp === 1 ? '' : 's'} detected · changing restarts the renderer (~2 s black)"><option value="1" ${omode === '1' ? 'selected' : ''}>HDMI 1</option><option value="2" ${omode === '2' ? 'selected' : ''}>HDMI 2</option><option value="mirror" ${omode === 'mirror' ? 'selected' : ''}>both · mirror</option><option value="dual" ${omode === 'dual' ? 'selected' : ''}>both · dual tiles</option></select>${ndisp < 2 && omode !== '1' ? '<span class="warn" title="asked for two outputs but only one display connected">1 disp</span>' : ''} `;
  return outSel + `<i class="hdot ${dot}" title="${esc(p.msg || 'not polled yet')}"></i> <span class="mono" style="font-size:11px">${esc(label)}${p.source ? ' · ' + esc(p.source) : ''}</span> <button class="btn small" data-pj="${esc(n)}" data-cmd="${pw === 'on' ? 'off' : 'on'}" title="${pw === 'on' ? 'power off' : 'power on'}">⏻</button><select class="btn small" data-pjin="${esc(n)}" title="input"><option value="">in…</option><option value="hdmi1">HDMI 1</option><option value="hdmi2">HDMI 2</option><option value="blank">blank</option><option value="unblank">unblank</option><option value="status">re-read</option></select><button class="btn small" data-pj="${esc(n)}" data-cmd="config" title="protocol / port">⚙</button>`;
}
async function outputsSet(name, mode, el) {
  if (!await ui.confirm(`${name} restarts its renderer (about 2 seconds of black on that projector).`, {title: `Switch ${name} to ${({1: 'HDMI 1', 2: 'HDMI 2', mirror: 'both ports, mirrored', dual: 'both ports as two tiles'})[mode]}?`, ok: 'Switch'})) { if (el) el.value = ((S.fleet[name] || {}).out || '1/1').split('/')[0]; return; }
  const r = await (await fetch(`/api/rig/${encodeURIComponent(name)}/outputs`, {method: 'POST', headers: {'content-type': 'application/json'}, body: JSON.stringify({mode})})).json();
  ui.toast(`${esc(name)}: ${esc(r.msg || (r.ok ? 'ok' : 'failed'))}`, r.ok ? 'ok' : 'bad', 4000);
}
