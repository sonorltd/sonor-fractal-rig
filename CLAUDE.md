...
- 2026-09-26 v0.8.0 (cont. 2) — **`web/js/ui.js` UI kit** (modal dialogs, saveAs, toasts, cards, tiles) replaces every
  ...
  **Mapping presets**: named copy of one projector's mapping (`master/mapping_presets/*.json`, cloud kind
  ...
  LED configs, mapping presets, cue lists (documents → cards) · looks, palettes, Milkdrop (quick recall → listBox).
  **Cue lists**: `master/cue_lists/*.json` {cues, mods}, kind `cuelist` (shared), `/api/cues/lists[/{name}[/load]]`;
  the LIVE stack is still cues.json / `cue_stack/current`. **Info tab**: `js/info.js` builds the side menu from
  `#info .card[id] > h3` (add a card with an id = it is in the menu), sections collapse (state in localStorage),
  search filters; new `#i-topology` card with the field-network SVG + address plan; intro fixed (31 params / 164 B).
...
- **Saved collections render one way.** Whole documents (shows, LED configurations, mapping presets, cue lists) → `ui.cards` on their tab and
...
- **Shared flows are single functions** used from every entry point: `showSaveDialog`, `ledcSaveDialog/ledcLoad`, `mappSaveDialog/mappLoad`, `cuelSaveDialog/cuelLoad`,
